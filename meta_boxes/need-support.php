...
class Schemeable_Sliding_Panel_v1_1a_m_Need_Support_ {
  static function setup() {
    $var = new Schemeable_Sliding_Panel_v1_1a_m_Need_Support_;
  ...
  function add_menu_page_meta_box( $meta_menu_page ) {
      if( $meta_menu_page == 'sliding-panel-options' ) {
        $this->__enqueue_css();
        add_meta_box( 
            'Schemeable_Sliding_Panel_v1_1a_m_Need_Support_',
            __( 'Need Support?', 'Schemeable Sliding Panel_textdomain' ),
  ...
  function get_field_id( $id ) {
    return 'id_Schemeable_Sliding_Panel_v1_1a_m_Need_Support__' . $id ;
  }

  function get_field_name( $id ) {
    return 'name_Schemeable_Sliding_Panel_v1_1a_m_Need_Support__' . $id ;
  }

  function render_meta_box( $post ) {

    wp_nonce_field( plugin_basename( __FILE__ ), 'Schemeable_Sliding_Panel_v1_1a_m_Need_Support__noncename' );

    $instance = get_post_meta( $post->ID, 'support', true );
    if( empty( $instance ) ) {
      $instance = array(
                 'need_support' => '',
                );
      $instance = apply_filters('pde-meta-box-defaults-support', $instance);
    }
    ?>
...
?>
      ...
      <div class="pde-form-field pde-form-markup markup-style-html">
        If you are having problems with this plugin, please talk about them in the <a href="http://wordpress.org/support/plugin/schemeable-sliding-panel/">Support forums</a>.
...
<?php 
  ...
  function save_post( $post_id ) {
    ...
    if ( !isset( $_POST['Schemeable_Sliding_Panel_v1_1a_m_Need_Support__noncename']) || !wp_verify_nonce( $_POST['Schemeable_Sliding_Panel_v1_1a_m_Need_Support__noncename'], plugin_basename( __FILE__ ) ) )
        return;
    ...
    $odefaults = apply_filters('pde-meta-box-defaults-support', $odefaults);
    $defaults = array();
    ...
    $instance = array();
      $instance['need_support'] = $post_vars[$this->get_field_name('') . 'need_support'] ;
    update_post_meta( $post_id, 'support', $instance );
  }
...
if( is_admin( ) )
  add_action( 'admin_init', array( 'Schemeable_Sliding_Panel_v1_1a_m_Need_Support_', 'setup' ) );
?>
