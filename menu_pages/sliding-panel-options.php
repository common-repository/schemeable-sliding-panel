...
class Schemeable_Sliding_Panel_v1_1a_u_Sliding_Panel_Options {
  static function setup() {
    $var = new Schemeable_Sliding_Panel_v1_1a_u_Sliding_Panel_Options;
  ...
  function get_field_id( $id ) {
    return 'id_Schemeable_Sliding_Panel_v1_1a_u_Sliding_Panel_Options_' . $id ;
  }

  function get_field_name( $id ) {
    return 'name_Schemeable_Sliding_Panel_v1_1a_u_Sliding_Panel_Options_' . $id ;
  }
  ...
  function render_menu_page() {
    ...
    if( empty( $instance ) || $action != 'edit' ) {
      ...
      $instance['use_widgets'] = $post_vars[$this->get_field_name('') . 'use_widgets'] ;
...
<div class="wrap" style="width:1120px;">
  ...
  <?php
    ...
    foreach( $messages as $message ) {
      ?><div class="<?php echo $message[0]; ?>"><p><?php _e( $message[1] ); ?></p></div><?php
    }
  ?>
  <br class="clear"/>
...
?>
    <div class="pde-form-field pde-form-text tab_background_color">
      ...
      <div class="pde-form-description">
        <label for="<?php echo $this->get_field_id('tab_background_color'); ?>">
          <span><?php _e( 'Color used for the background of the tab.' ); ?></span>
        </label>
      </div>
...
?>
    <div class="pde-form-field pde-form-text tab_border_color">
      ...
      <div class="pde-form-description">
        <label for="<?php echo $this->get_field_id('tab_border_color'); ?>">
          <span><?php _e( 'Color used for the border of the tab.' ); ?></span>
        </label>
      </div>
...
?>
      ...
      <div class="pde-form-field pde-form-markup markup-style-html">
        <?php else: ?>
<p>You could use openID to let users register/login into the site, if <a href="http://wordpress.org/extend/plugins/wordpress-social-login/">Wordpress Social Login</a> plugin is installed.</p>
<?php endif; ?>
...
<?php
?>
    <div class="pde-form-field pde-form-checkbox use_widgets">
        <div class="pde-form-title">
          <label for="<?php echo $this->get_field_id('use_widgets'); ?>">
            <span><?php esc_html_e( __('Use Widgets') ); ?></span>
          </label>
        </div>
        <div class="pde-form-input">
          <input class="wp-pde-checkbox" id="<?php echo $this->get_field_id('use_widgets'); ?>"
            value="Use Widgets"
            name="cb-<?php echo $this->get_field_name('use_widgets'); ?>"
            type="checkbox"<?php checked(isset($instance['use_widgets']) ? $instance['use_widgets'] : '', 'Use Widgets'); ?> />
          <input id="txtcb-<?php echo $this->get_field_id('use_widgets'); ?>"
            value="Use Widgets"
            name="<?php echo $this->get_field_name('use_widgets'); ?>"
            type="hidden" />
<script type="text/javascript">
(function($) {
  $('#<?php echo $this->get_field_id('use_widgets'); ?>').change(function(e) {
    if($(this).attr('checked'))
      $('#txtcb-<?php echo $this->get_field_id('use_widgets'); ?>').val($(this).val());
    else
      $('#txtcb-<?php echo $this->get_field_id('use_widgets'); ?>').val('');
  });
})(jQuery);
</script>
          <label for="<?php echo $this->get_field_id('use_widgets'); ?>">
...
<?php 
?>
    <div class="display_when_unselected group-for-checkbox group-use_widgets" id="group-<?php echo $this->get_field_id("use_widgets") ?>">
<script type="text/javascript">
(function($) {
  $(document).ready(function(e) {
    $('.wp-pde-checkbox').trigger('change');
...
add_action('admin_menu', array('Schemeable_Sliding_Panel_v1_1a_u_Sliding_Panel_Options', 'setup'));
