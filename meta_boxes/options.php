...
class Schemeable_Sliding_Panel_v1_1a_m_Options {
  static function setup() {
    $var = new Schemeable_Sliding_Panel_v1_1a_m_Options;
  ...
  function add_menu_page_meta_box( $meta_menu_page ) {
      if( $meta_menu_page == 'sliding-panel-options' ) {
        $this->__enqueue_css();
        add_meta_box( 
            'Schemeable_Sliding_Panel_v1_1a_m_Options',
  ...
  function get_field_id( $id ) {
    return 'id_Schemeable_Sliding_Panel_v1_1a_m_Options_' . $id ;
  }

  function get_field_name( $id ) {
    return 'name_Schemeable_Sliding_Panel_v1_1a_m_Options_' . $id ;
  }

  function render_meta_box( $post ) {

    wp_nonce_field( plugin_basename( __FILE__ ), 'Schemeable_Sliding_Panel_v1_1a_m_Options_noncename' );
...
<?php 
    $scheme_name = esc_attr( $instance['scheme_name'] );
?>
    <div class="pde-form-field pde-form-text scheme_name">
      <div class="pde-form-title">
        <label for="<?php echo $this->get_field_id('scheme_name'); ?>">
          <span><?php esc_html_e( __('Scheme Name') ); ?></span>
        </label>
      </div>
      <div class="pde-form-input">
        <input type="text" value="<?php echo $scheme_name; ?>" name="<?php echo $this->get_field_name('scheme_name'); ?>" id="<?php echo $this->get_field_id('scheme_name'); ?>" />
      </div>
      <div class="pde-form-description">
        <label for="<?php echo $this->get_field_id('scheme_name'); ?>">
          <span><?php _e( 'Enter a name for the scheme and click on Save to save the scheme locally.' ); ?></span>
        </label>
      </div>
    </div> <!-- scheme_name -->
...
<?php 
  ...
  function save_post( $post_id ) {
    ...
    if ( !isset( $_POST['Schemeable_Sliding_Panel_v1_1a_m_Options_noncename']) || !wp_verify_nonce( $_POST['Schemeable_Sliding_Panel_v1_1a_m_Options_noncename'], plugin_basename( __FILE__ ) ) )
        return;
    ...
    $instance = array();
      ...
      $instance['scheme_name'] = strip_tags( $post_vars[$this->get_field_name('') . 'scheme_name'] );
...
if( is_admin( ) )
  add_action( 'admin_init', array( 'Schemeable_Sliding_Panel_v1_1a_m_Options', 'setup' ) );
?>
